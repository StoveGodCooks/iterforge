...
def _as_side_facing(kps: list[Keypoint]) -> list[Keypoint]:
    """
    Convert a front skeleton to a right-facing PROFILE (side) view.

    A profile collapses the body's width to a near-single vertical axis (depth
    becomes the off-screen axis), the nose points to one side, and the far-side
    eye/ear are occluded. Near-side (right) limbs stay full confidence; far-side
    (left) limbs overlap them on the same column at reduced confidence so the
    silhouette reads as an unambiguous side view for ControlNet.
    """
    out = list(kps)
    # Head faces right: nose ahead of the face line; far (left) eye/ear hidden.
    out[0]  = (0.560, 0.130, 1.0)  # nose forward (right)
    out[14] = (0.535, 0.115, 1.0)  # r_eye (near) visible
    out[15] = (0.520, 0.115, 0.0)  # l_eye (far) hidden
    out[16] = (0.470, 0.125, 1.0)  # r_ear (near) visible
    out[17] = (0.490, 0.125, 0.0)  # l_ear (far) hidden
    out[1]  = (0.500, 0.220, 1.0)  # neck on axis
    # Torso collapses to a narrow column around 0.50; near-side limbs ahead,
    # far-side tucked behind on the same column at lower confidence.
    out[2]  = (0.500, 0.230, 1.0)  # r_shoulder (near) on axis
    out[3]  = (0.505, 0.370, 1.0)  # r_elbow
    out[4]  = (0.510, 0.510, 1.0)  # r_wrist
    out[5]  = (0.490, 0.230, 0.6)  # l_shoulder (far, faint)
    out[6]  = (0.485, 0.370, 0.6)  # l_elbow (far)
    out[7]  = (0.480, 0.510, 0.6)  # l_wrist (far)
    out[8]  = (0.500, 0.520, 1.0)  # r_hip (near) on axis
    out[9]  = (0.505, 0.700, 1.0)  # r_knee
    out[10] = (0.512, 0.885, 1.0)  # r_ankle
    out[11] = (0.490, 0.520, 0.6)  # l_hip (far)
    out[12] = (0.485, 0.700, 0.6)  # l_knee (far)
    out[13] = (0.480, 0.885, 0.6)  # l_ankle (far)
    return out


# ── Direction-aware negative prompt add-ons (S5) ──────────────
# The worker appends these to the negative prompt based on a pose's direction,
# so a back frame can't grow a face and a side frame can't snap frontal.
BACK_POSE_NEGATIVE = "face, facial features, eyes, nose, mouth, front view, looking at camera"
SIDE_POSE_NEGATIVE = "front view, facing camera directly, three-quarter view"


def direction_negative(direction: str) -> str:
    """Return the extra negative-prompt fragment for a pose direction (may be empty)."""
    if direction == "back":
        return BACK_POSE_NEGATIVE
    if direction == "side":
        return SIDE_POSE_NEGATIVE
    return ""
...
PRESETS: tuple[PosePreset, ...] = (
    ...
    _preset(
        "idle_side", "Idle — Side", "side",
        "standing idle, side profile view, facing right, full body shown in profile",
        _as_side_facing(_neutral),
    ),
    _preset(
        "walk_side_a", "Walk — Side A", "side",
        "walk cycle, side profile view, facing right, leading leg forward, mid-stride",
        _as_side_facing(_walk_step(_neutral, +1.0)),
    ),
    _preset(
        "walk_side_b", "Walk — Side B", "side",
        "walk cycle, side profile view, facing right, trailing leg forward, mid-stride",
        _as_side_facing(_walk_step(_neutral, -1.0)),
    ),
)
