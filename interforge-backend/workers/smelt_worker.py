...
async def run_smelt_all_views(job: Job, params: dict) -> None:
    ...
    from inference.zero123 import Zero123Engine, _MODEL_DIR, _HF_MODEL_ID

    cached, cache_msg = _probe_model_cache(_MODEL_DIR, _HF_MODEL_ID, "model_index.json")
    await job.push(log_event(f"Zero123++ weights: {cache_msg}"))
    if not cached:
        await job.push(log_event(
            "FIRST-RUN download ~3GB — expect 3–10 min over typical broadband. "
            "Heartbeat ticks below mean the job is alive, not frozen."
        ))
    ...
    load_msg = (
        "Loading Zero123++ from cache"
        if cached
        else "Loading Zero123++ (first-run download in progress)"
    )
    try:
        async with _heartbeat(job, load_msg):
    ...
    try:
        # ── 3. Generate 6 views ──────────────────────────────────
        await job.push(log_event("Generating 6 views with Zero123++ (28 steps)…"))

        async with _heartbeat(job, "Zero123++ diffusion running"):
    ...
    # ── 5. Save + luminance-key each view ────────────────────
    # Zero123++ prints onto a known flat gray (127,127,127). A luminance
    # key on that color is ~60× faster than U2Net and produces cleaner
    # edges (no saliency mis-classification on dark silhouette pixels).
    await job.push(log_event("Keying gray background from all views…"))
    ...
    for view_name in VIEW_ORDER:
        ...
        # Luminance key against Zero123++'s training gray
        with profiler.section(f"luma_{view_name}", f"luminance key — {view_name}"):
            try:
                raw_bytes = raw_out.read_bytes()
                processed = await save_and_process_view_luma(
                    raw_png=raw_bytes,
                    out_dir=out_dir,
                    index=0,
                ...
                rembg_fail_count += 1
                await job.push(log_event(f"luma key failed for {view_name} (non-fatal): {exc}"))
                processed = {
                    "index": 0,
...
# ── Sprite-sheet prompt helpers ───────────────────────────────
#
# Negative prompts branch by asset category: characters need a body, so
# anatomy guards stay relaxed; objects hard-ban human anatomy so the
# IP-Adapter reference can't leak a wielder into the frame.
...
def _sort_components_by_x(component_pngs: list[bytes]) -> list[bytes]:
    """
    Re-sort a list of component PNGs by the x-centroid of their alpha.
    Used after we filter to the two largest, to restore left→right order.
    """
    import io
    from PIL import Image as _PIL
    import numpy as np

    with_centroid: list[tuple[float, bytes]] = []
    for b in component_pngs:
        img = _PIL.open(io.BytesIO(b)).convert("RGBA")
        alpha = np.array(img)[:, :, 3]
        ys, xs = np.where(alpha > 32)
        cx = float(xs.mean()) if len(xs) else 0.0
        with_centroid.append((cx, b))
    with_centroid.sort(key=lambda e: e[0])
    return [b for _, b in with_centroid]
