...
const MANAGED_INKSCAPE_EXE = path.join(ITERFORGE_HOME, 'inkscape', 'bin', 'inkscape.exe');

const router = express.Router();

router.get('/', async (_req, res) => {
  try {
    const [comfyHealth, env, comfyInstalled, blenderInstalled, inkscapeInstalled, ipAdapterReady, controlNet, upscalerReady] = await Promise.all([
      healthCheck(),
      readEnv(),
      isComfyInstalled(),
      fs.pathExists(MANAGED_BLENDER_EXE),
      fs.pathExists(MANAGED_INKSCAPE_EXE),
    ...
    res.json({
      ...
      inkscapeInstalled,
      inkscapeVersion:   env.tools?.inkscape?.version ?? null,
      setup:             getSetupState(),
      tier:              env.tier ?? 'local',
      version:           '1.0.0',
      ipAdapterReady,
